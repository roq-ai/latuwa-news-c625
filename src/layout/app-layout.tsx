...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiFileText, FiRadio } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['news-editor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BroadcasterInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/broadcasters` : null),
    () => getBroadcasters({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Broadcaster',
      path: isTenantUser ? `/broadcasters/view/${data?.[0]?.id}` : '/broadcasters',
      entity: 'broadcaster',
